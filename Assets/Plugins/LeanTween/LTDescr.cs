...
public class LTDescr
{
	...
	*/
	...
	public bool updateInternal(){
		...
		// GG-fix (Mikael):
		// The original logic could leave the dt value as-is, since there was no else-branch in the
		// following checks. I commented out the first check and added an else-branch at the end,
		// meaning that if no instructions have been set for the use of delta time, the tween will
		// default to LeanTween.dtActual.
		//
		// This is a fix for an issue where the line "dt = dt*directionLocal;" changed the value to
		// be negative, causing the next tween subtract a negative delta time from its delay. This
		// prevented some tweens from starting until another tween was updated that hopefully fixed
		// the delta time to be positive.
